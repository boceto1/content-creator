...
def createReel(clipWithInfo, endVideo, isPreview):
...
def createReels(baseVideo, clipsInfo, endVideo, isPreview):
  clips = createClips(baseVideo, clipsInfo)
  clipsWithInfo = zip(clips, clipsInfo)
  reels = map(lambda clipWithInfo: createReel(clipWithInfo, endVideo, isPreview), clipsWithInfo)
  return reels
...
def generateReels(projectData, isPreview):
  ...
  reels = createReels(resizedVideo, projectData['clips'], endClip, isPreview)
